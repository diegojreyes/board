...
void user_set_debounce(uint32_t debounce_press_ms,uint32_t debounce_release_ms)
{
    ...
    LOG_DBG("set debounce,scan:%d,press:%d,release:%d",user_debounce.debounce_scan_period_ms,debounce_press_ms,debounce_release_ms);
}
void user_set_scan_period(uint32_t period)
{
    user_debounce.debounce_scan_period_ms = period;
}
uint32_t get_default_scan_period(void)
{
    return user_debounce.default_scan_period;
}
#endif 
/**
...
static int kscan_matrix_read(const struct device *dev)
{
    ...
    for (int col = 0; col < config->outputs.len; col++) {
        ...
        port = (GPIOB->GPIO_PAD_STATE&0x7ffcffff)  | (GPIOA->GPIO_PAD_STATE &((1<<16)|(1<<17)|(1<<31)));
#elif CONFIG_SHIELD_KEYCHRON_V5ULTRA_ANSI
        port = (GPIOB->GPIO_PAD_STATE&0xfff3ff7f) | ((GPIOA->GPIO_PAD_STATE &((1<<16)|(1<<27)|(1<<28)))>>9);
        ...
        port = (GPIOB->GPIO_PAD_STATE&0xfff3feff) | ((GPIOA->GPIO_PAD_STATE &((1<<17)|(1<<27)|(1<<28)))>>9);
#elif (CONFIG_SHIELD_KEYCHRON_QN104_ANSI || CONFIG_SHIELD_KEYCHRON_Q6ULTRA_ANSI || CONFIG_SHIELD_KEYCHRON_V6ULTRA_ANSI|| CONFIG_SHIELD_KEYCHRON_V3ULTRA_ANSI ||CONFIG_SHIELD_KEYCHRON_V10ULTRA_ANSI)
        port = (GPIOB->GPIO_PAD_STATE&0xfff3fe7f) | ((GPIOA->GPIO_PAD_STATE &((1<<16)|(1<<17)|(1<<27)|(1<<28)))>>9);
#elif CONFIG_SHIELD_KEYCHRON_V0ULTRA_ANSI  
...
#if (CONFIG_SHIELD_KEYCHRON_V5ULTRA_ANSI ||CONFIG_SHIELD_KEYCHRON_QN104_ANSI || CONFIG_SHIELD_KEYCHRON_V2ULTRA_ANSI || CONFIG_SHIELD_KEYCHRON_Q6ULTRA_ANSI || CONFIG_SHIELD_KEYCHRON_V6ULTRA_ANSI || CONFIG_SHIELD_KEYCHRON_V3ULTRA_ANSI ||CONFIG_SHIELD_KEYCHRON_V10ULTRA_ANSI )        
            //fix pin 
...
#if CONFIG_LAUNCHER_USER_SET_DEBOUNCE
#if CONFIG_FAST_KSCAN
            zmk_debounce_update(&data->matrix_state[index], active, elapsed_ms,&user_debounce.debounce_config);
#else
    ...
    case PM_DEVICE_ACTION_SUSPEND:
        ...
        if(!data->press_rows)
        {
...
#endif             
            {
                ...
                if(wakeup_flag) {
                    // LOG_ERR("i:%d,pin:%d,wakeup high:%d,low:%d",i,state->pins[i].pin,state->pins[i].wakeup_high,state->pins[i].wakeup_low);
                    if (data->press_rows & BIT(i)) {
                        if (state->pins[i].wakeup_high) {
                            // LOG_DBG("wake high , row%d, pin%d",i-config->rows,state->pins[i].pin);
                            Pad_SetControlMode(state->pins[i].pin, PAD_SW_MODE);
                            System_WakeUpPinEnable(state->pins[i].pin, PAD_WAKEUP_POL_LOW, DISABLE);
                            Pad_Config(state->pins[i].pin, PAD_SW_MODE, PAD_IS_PWRON, PAD_PULL_UP, state->pins[i].dir, state->pins[i].drive);
                            ...
                            Pad_Config(state->pins[i].pin, PAD_SW_MODE, PAD_IS_PWRON, PAD_PULL_DOWN, state->pins[i].dir, state->pins[i].drive);
                        }
                    } else {
                        // LOG_ERR("xx");
                        if (state->pins[i].wakeup_high) {
...
//     for (int i = 0; i < data->inputs.len; i++) {
//         const struct gpio_dt_spec *gpio = &data->inputs.gpios[i].spec;
//         gpio_pin_interrupt_configure_dt(gpio, (wakeup_pins & (1<<i))?GPIO_INT_LEVEL_INACTIVE:GPIO_INT_LEVEL_ACTIVE);
