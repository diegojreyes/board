...
int sn3734_write(const struct device *dev , uint8_t page, uint8_t reg, uint8_t *p_data, uint8_t len) {
    const struct sn3734_config *config = dev->config;
...
static inline int sn3734_write_register(const struct device *dev,uint8_t page,uint8_t reg,uint8_t value)
{
    uint8_t buf[3]={SN3734_WRITE | SN3734_ID | (page & 0x0F),reg,value};
    return spi_write_datas(dev,buf,3);
}
static inline int sn3734_read_register(const struct device *dev,uint8_t page,uint8_t reg,uint8_t *value)
{
    const struct sn3734_config *config = dev->config;
...
static inline int sn3734_spi_flush_pwm_buffer(const struct device *dev) {
    ...
    int ret =sn3734_write(dev, SN3734_PWM_PAGE, SN3734_PWM_PAGE_PWM_START, pwm, SN3734_PWM_PAGE_REGISTER_COUNT);
    ret =sn3734_write(dev, SN3734_FUNC_PAGE, SN3734_FUNC_PAGE_PWM_START, pwm + SN3734_PWM_PAGE_REGISTER_COUNT, SN3734_FUNC_PAGE_REGISTER_COUNT);
...
static int sn3734_update_rgb(const struct device *dev, struct led_rgb *pixels,
                                size_t num_pixels) {
    const struct sn3734_config *config = dev->config;
...
static int sn3734_init(const struct device *dev) {
...

    {
        ...
        // Set to normal mode
        sn3734_write_register(dev, SN3734_FUNC_PAGE, SN3734_FUNC_REG_CONFIGURATION, SN3734_NORMAL_MODE | SN3734_PWM_MODE_8 | SN3734_SWS);
        // Set Golbal Current Control Register
        sn3734_write_register(dev, SN3734_FUNC_PAGE, SN3734_FUNC_REG_GLOBAL_CURRENT, SN3734_GLOBAL_CURRENT);
        // Set Pull up & Down for SWx CSy
        sn3734_write_register(dev, SN3734_FUNC_PAGE, SN3734_FUNC_REG_PULLDOWNUP, SN3734_SWPDR | SN3734_CSPUR);
...
#define SN3734_INIT(n)                                                                          \
    ...
    DEVICE_DT_INST_DEFINE(n, &sn3734_init, PM_DEVICE_DT_INST_GET(n), NULL, &sn3734_config_##n, POST_KERNEL,      \
                          CONFIG_SN3734_INIT_PRIORITY, &sn3734_api);
