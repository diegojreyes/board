...
static inline int ckled2001_spi_write_reg(const struct device *dev, uint8_t page, uint8_t reg,
                                          uint8_t value) {
    uint8_t buf[3] = {0x20 | page, reg, value};
    return spi_write_datas(dev, buf, 3);
}
static inline int ckled2001_spi_read_reg(const struct device *dev, uint8_t page, uint8_t reg,
                                         uint8_t *value) {
    const struct ckled2001_config *config = dev->config;
...
static inline int ckled2001_spi_flush_pwm_buffer(const struct device *dev) {
    ...
    struct spi_buf tx_buf[1] = {{.buf = cmd, .len = 2 + LED_PWM_CNT}};
    ...
    // return 0;
    // // LOG_ERR("set cs
    // pin:%d,port:%p,flag:%x",spi_dt->config.cs.gpio.pin,spi_dt->config.cs.gpio.port,spi_dt->config.cs.gpio.dt_flags);
}
#else
...
static int ckled2001_init(const struct device *dev) {
    ...
    // if (chip_id == 0x8a)
    {
        ...
        ckled2001_spi_write_reg(dev, FUNCTION_PAGE, REG_SCAN_PHASE,
                                MSKPHASE_CHANNELS(config->scan_phase_channels));
        ckled2001_spi_write_reg(dev, FUNCTION_PAGE, REG_SLEW_RATE_CONTROL_MODE1,
                                MSKPWM_DELAY_PHASE_ENABLE);
...
#define CKLED2001_INIT(n)                                                                          \
    ...
    DEVICE_DT_INST_DEFINE(n, &ckled2001_init, PM_DEVICE_DT_INST_GET(n), NULL,                      \
                          &ckled2001_config_##n, POST_KERNEL, CONFIG_CKLED2001_INIT_PRIORITY,      \
                          &ckled2001_api);
