...
static uint8_t discover_func(struct bt_conn *conn, const struct bt_gatt_attr *attr,
    ...
    } else if (!bt_uuid_cmp(discover_params.uuid, BT_UUID_DIS_MANUFACTURER_NAME)) {
    ...
    } else if (!bt_uuid_cmp(discover_params.uuid, BT_UUID_DIS_PNP_ID)) {
...
uint8_t get_os_type(void) { return os_type; }