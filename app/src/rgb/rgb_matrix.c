...
__attribute__((weak)) uint8_t zmk_rgb_matrix_map_row_column_to_led_kb(uint8_t row, uint8_t column, uint8_t *led_i) {
    return 0;
}
...
static void zmk_rgb_task_render(uint8_t effect) {
    ...
    if(effect<RGB_EFFECT_MAX )///UINT8_MAX)
    {
...
void zmk_rgb_matrix_task(void) {
    ...
    uint8_t effect = (suspend_backlight || (!rgb_matrix_config.enable && !rgb_led_indicators.rgb_enable)) ? 0 : rgb_matrix_config.mode;
    ...
    if(back_effect !=effect)
    {
        ...
        {       
            if(back_rgb_config.raw ==0){
                ...
                LOG_ERR("back mode:%d,hsv:%x,%x,%x",rgb_matrix_config.mode,rgb_matrix_config.hsv.h,rgb_matrix_config.hsv.s,rgb_matrix_config.hsv.v);
            }
        }
        else if(back_effect ==RGB_EFFECT_MIXED_RGB )
        {
            if((effect>0 && effect !=0x3f)&& (back_rgb_config.raw!=0))
            {
                ...
                LOG_ERR("restore mode:%d,hsv:%x,%x,%x",rgb_matrix_config.mode,rgb_matrix_config.hsv.h,rgb_matrix_config.hsv.s,rgb_matrix_config.hsv.v);
            }
        ...
        back_effect =effect;        
        rgb_last_effect =0;
        LOG_ERR("change effect:%d,back:%d,last:%d,enable:%d",effect,back_effect,rgb_last_effect,rgb_matrix_config.enable);
    }
    switch (rgb_task_state) {
        ...
        case RGB_STATE_RENDER:
            ...
            if (effect) {
                if (rgb_task_state == RGB_STATE_FLUSH) { // ensure we only draw basic indicators once rendering is finished
...
void zmk_rgb_matrix_toggle_no_save(void) {
    ...
    LOG_DBG("rgb matrix toggle:enable = %u,mode:%d,%d\n",rgb_matrix_config.enable,rgb_matrix_config.mode,rgb_matrix_config.back_mode);
...
void zmk_rgb_matrix_enable(void) {
    if (!rgb_matrix_config.enable) rgb_task_state = RGB_STATE_START;
    rgb_matrix_config.enable = 1;
    save_rgb_matrix_config();
    rgb_onoff_status = 1;
}
...
void zmk_rgb_matrix_increase_hue(void) {
    zmk_rgb_matrix_sethsv(rgb_matrix_config.hsv.h + RGB_MATRIX_HUE_STEP, rgb_matrix_config.hsv.s, rgb_matrix_config.hsv.v);
}
...
void zmk_rgb_matrix_increase_sat(void) {
        zmk_rgb_matrix_sethsv(rgb_matrix_config.hsv.h, qadd8(rgb_matrix_config.hsv.s, RGB_MATRIX_SAT_STEP), rgb_matrix_config.hsv.v);
}

void zmk_rgb_matrix_decrease_sat(void) {
    zmk_rgb_matrix_sethsv(rgb_matrix_config.hsv.h, qsub8(rgb_matrix_config.hsv.s, RGB_MATRIX_SAT_STEP), rgb_matrix_config.hsv.v);

}

void zmk_rgb_matrix_increase_val(void) {
    zmk_rgb_matrix_sethsv(rgb_matrix_config.hsv.h, rgb_matrix_config.hsv.s, qadd8(rgb_matrix_config.hsv.v, RGB_MATRIX_VAL_STEP));
#ifdef CONFIG_ZMK_SSD1306    
...
void zmk_rgb_matrix_decrease_val(void) {
    zmk_rgb_matrix_sethsv(rgb_matrix_config.hsv.h, rgb_matrix_config.hsv.s, qsub8(rgb_matrix_config.hsv.v, RGB_MATRIX_VAL_STEP));
#ifdef CONFIG_ZMK_SSD1306     
    uint8_t index = (rgb_matrix_config.hsv.v+RGB_MATRIX_VAL_STEP/2) /RGB_MATRIX_VAL_STEP;
    disp_change_setting(TYPE_LIGHTNESS,index);
#endif 
...
void zmk_rgb_matrix_set_state(uint8_t state)
{
    rgb_task_state = state;
}
