...
void snap_click_config_reset(void) {
    ...
    // eeprom_update_block(snap_click_pair, (uint8_t *)(EECONFIG_BASE_SNAP_CLICK),
    // sizeof(snap_click_pair));
}

void snap_click_init(void) {
    // eeprom_read_block(snap_click_pair, (uint8_t *)(EECONFIG_BASE_SNAP_CLICK),
    // sizeof(snap_click_pair));
...
static bool snap_click_get(uint8_t *data) {
    ...
    if (count > 9 || start + count > SNAP_CLICK_COUNT)
        return false;
...
static bool snap_click_save(uint8_t *data) {
    settings_save_one("keychron/snap_click_pair", (const void *)snap_click_pair,
                      sizeof(snap_click_pair));
