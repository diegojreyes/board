...
static void save_state_worker(struct k_work *work) {
    ...
    if(rgb_matrix_config .mode == RGB_EFFECT_MIXED_RGB)
    {
        ...
        LOG_ERR("cur hsv:%x,%x,%x,spd:%d",back_rgb_config.hsv.h,back_rgb_config.hsv.s,back_rgb_config.hsv.v,back_rgb_config.speed);
        settings_save_one("rgb/underglow/state", &back_rgb_config, sizeof(back_rgb_config));
        return;
    }
#endif 
    LOG_ERR("save rgb:%x,%x,%x,mode:%d",rgb_matrix_config.hsv.h,rgb_matrix_config.hsv.s,rgb_matrix_config.hsv.v,rgb_matrix_config.mode);
...
{
    if(rgb_matrix_device==NULL) return ;
...
void zmk_rgb_matrix_driver_init(void)
{
    ...
    // int rc=load_immediate_value("rgb/underglow/state",&rgb_matrix_config,sizeof(rgb_matrix_config));
...
{
    if(!rgb_control_enable) return -1;
...
static int rgb_underglow_event_listener(const zmk_event_t *eh) {
    ...
    else
    {
        if (as_zmk_usb_conn_state_changed(eh)) {
            ...
            if(rgb_onoff_status && usb_configured && zmk_endpoints_selected().transport==ZMK_TRANSPORT_USB)
            {
                return rgb_underglow_auto_state(zmk_usb_get_status()!=USB_DC_SUSPEND);
            }                
            else
            {
                if((zmk_usb_get_status()== USB_DC_UNKNOWN) || (zmk_usb_get_status()== USB_DC_SUSPEND))
                {
                    zmk_rgb_matrix_off();
                    // rgb_onoff_status=0;   
...
K_THREAD_DEFINE(zmk_rgb_thread_tid, RGB_THREAD_STACK_SIZE,
                zmk_rgb_thread, NULL, NULL, NULL,
...
void zmk_rgb_thread(void)
{
    ...
    while(1)
    {
        ...
        if((!zmk_usb_power_on()&& (bat_is_low()&& !rgb_led_indicators.rgb_enable && k_uptime_get_32()>8000)))
        {
            LOG_ERR("bat low,no rgb inicator");
        }    
        if((!rgb_matrix_config.enable && !rgb_led_indicators.rgb_enable) 
            || (bat_is_shutdown()&&!zmk_usb_power_on())
            || (!zmk_usb_power_on()&& (bat_is_low()&& !rgb_led_indicators.rgb_enable && k_uptime_get_32()>3000)))
        {
...
{
    return rgb_enable_dlps;
}

uint8_t get_rgb_test_start(void)
{
    return rgb_test_start;
}