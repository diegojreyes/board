...
uint8_t zmk_usb_power_on(void) { return usb_power_on; }
...
#ifdef CONFIG_ENABLE_WIN_LOCK_INDICATOR
            if (zmk_ble_is_ready() || zmk_ppt_is_ready() ||
...
static int on_keymap_binding_released(struct zmk_behavior_binding *binding,
    ...
    case MOD_BLE:
        ...
        raise_zmk_led_state_changed((struct zmk_led_state_changed){
            .led_state = LED_PEER_STATE_POWR_OFF, .transport = ZMK_TRANSPORT_BLE});
        // hardware_select_transport=ZMK_TRANSPORT_USB;
        // zmk_endpoints_select_transport(hardware_select_transport);
        LOG_PANIC();
#if CONFIG_LED_STRIP
    ...
    case MOD_PPT:
        ...
        zmk_endpoints_send_report(HID_USAGE_KEY);
        raise_zmk_led_state_changed((struct zmk_led_state_changed){
            .led_state = LED_PEER_STATE_POWR_OFF, .transport = ZMK_TRANSPORT_PPT});
        // hardware_select_transport=ZMK_TRANSPORT_USB;
        // zmk_endpoints_select_transport(hardware_select_transport);
        LOG_PANIC();
#if CONFIG_LED_STRIP
...
enum zmk_transport get_hardware_select_transport(void) { return hardware_select_transport; }
bool is_ble(void) { return hardware_select_transport == ZMK_TRANSPORT_BLE; }
...
static int mode_handle_set(const char *name, size_t len, settings_read_cb read_cb, void *cb_arg);
...
void save_mode(enum zmk_transport mod) {
    int rc;
    if (mod == stored_transport)
        return;
    stored_transport = mod;
    rc = settings_save_one("savemode/mode", (const void *)&stored_transport,
                           sizeof(stored_transport));
    if (rc) {
...
void mode_init(void) {
    ...
    if (hardware_select_transport != ZMK_TRANSPORT_USB && hardware_select_transport != 0xff) {
        if (hardware_select_transport == ZMK_TRANSPORT_BLE)
            change_to_ble(0);
...
uint8_t winlock_led_is_on(void) { return winlock_led_on; }
void winlock_led_onoff(uint8_t onoff) {
...
#ifdef CONFIG_ZMK_SSD1306
void winlock_disp_cb(struct k_work *work) { disp_set_winlock(fn_win_lock); }
...
uint8_t get_charge_state(void) { return bat_charge_state; }