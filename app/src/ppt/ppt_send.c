...
void ringbuf_reset(void) { ring_buf_reset(&zmk_ppt_msgs); }
int ringbuf_used_get(void) { return ring_buf_size_get(&zmk_ppt_msgs) / (sizeof(zmk_ppt_report_t)); }
            ...
            for (int i = 1; i < 4; i++) {
                if (ringbuf_used_get()) {
                    ...
                    if ((opcode == SYNC_OPCODE_STD_KEYBOARD_ENCRYPTION_DATA ||
                         ...
                         opcode == SYNC_OPCODE_KEYBOARD || opcode == SYNC_OPCODE_ALL_KEYBOARD) &&
                        report[i].packet.opcode == opcode) {
