                                ...
                                T_KEYBOARD_DATA keyboard_data)
{
    ...
    if (keyboard_data.hid_report_type == HID_REPORT_KEYBOARD)
    {
        ...
        ret = ppt_app_send_data(type, msg_retrans_count, tx_keyboard_data, KEYBOARD_DATA_SIZE + 1+3);
    ...
    if (keyboard_data.hid_report_type == HID_REPORT_CONSUMER)
    {
        ...
        ret = ppt_app_send_data(type, msg_retrans_count, tx_comsumer_data, CONSUMER_DATA_SIZE + 1+3);
        if (ret == SYNC_ERR_CODE_SUCCESS)
        {
...
void zmk_ppt_init(void)
{
    ...
    if(bat_is_shutdown()&& !zmk_usb_power_on()) return;
    if(inited) 
    {
        // keyboard_ppt_enable();
        zmk_ppt_reconn();
...
{
    return ppt_app_global_data.keyboard_ppt_status;
}
...
void z_impl_z_log_msg_static_create(const void * source, const struct log_msg_desc desc, uint8_t * package, const void * data)
{
    return;
}
#endif 
