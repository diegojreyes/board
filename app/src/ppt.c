...
static void ppt_app_send_msg_cb(sync_msg_type_t type, uint8_t *p_data, uint16_t len,
    ...
    LOG_DBG("ppt_app_send_msg_cb, type: %d, len: %d, send_result: %d!", type, len, info->res);
}
int zmk_ppt_send_keyboard_report(uint8_t *report, uint8_t len) {
    ...
    const uint8_t kb_pub_usekeys[16] = {0x2c, 0xfa, 0xf3, 0x02, 0x6d, 0xa0, 0x48, 0x04,
                                        0x82, 0x59, 0x90, 0x4d, 0x7a, 0xe1, 0xfa, 0x03};
    ...
    // return 0;
    return sync_msg_send(SYNC_MSG_TYPE_INFINITE_RETRANS, tx_keyboard_data, index,
                         ppt_app_send_msg_cb); // SYNC_MSG_TYPE_DYNAMIC_RETRANS
    // return
    // ppt_app_send_data(SYNC_MSG_TYPE_DYNAMIC_RETRANS,0,report,(len+1+3));//zmk_ppt_send_report(tx_keyboard_data,
    // (len+1+3));
}
...
int zmk_ppt_send_consumer_report(uint8_t *report, uint8_t len) {
    ...
    return sync_msg_send(SYNC_MSG_TYPE_DYNAMIC_RETRANS, tx_keyboard_data, (len + 1 + 3),
                         ppt_app_send_msg_cb);
    // return
    // ppt_app_send_data(SYNC_MSG_TYPE_DYNAMIC_RETRANS,0,report,(len+1+3));//zmk_ppt_send_report(tx_keyboard_data,
    // (len+1+3));
