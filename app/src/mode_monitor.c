...
int zmk_mode_monitor_init(void) {
    ...
    LOG_WRN("ppt port:%s,pin:%d,val:%d",ppt_irq.port->name,ppt_irq.pin,gpio_pin_get_raw(ppt_irq.port,ppt_irq.pin));
    ...
    LOG_WRN("bt port:%s,pin:%d,val:%d",bt_irq.port->name,bt_irq.pin,gpio_pin_get_raw(bt_irq.port,bt_irq.pin));
    ...
    LOG_WRN("winmac port:%s,pin:%d,val:%d",win2mac.port->name,win2mac.pin,gpio_pin_get_raw(win2mac.port,win2mac.pin));
    ...
    LOG_WRN("detect_usb port:%s,pin:%d,val:%d",detect_usb.port->name,detect_usb.pin,gpio_pin_get_raw(detect_usb.port,detect_usb.pin));
