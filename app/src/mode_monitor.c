...
void generate_key(uint32_t row, uint32_t column, bool pressed);
...
static bool usb_mode_monitor_trigger_level = GPIO_PIN_LEVEL_LOW;
...
// T_APP_GLOBAL_DATA app_global_data = {.is_app_enabled_dlps = true, .is_usb_enumeration_success =
// false};
...
static void zmk_mode_monitor_callback(const struct device *dev, struct gpio_callback *gpio_cb,
                                      uint32_t pins) {
    LOG_DBG("zmk_mode_monitor_callback, pins is %x,", pins);
    ...
    if ((!strcmp(dev->name, ppt_irq.port->name)) && (pins >> ppt_irq.pin == 1)) {
        if (app_mode.is_in_ppt_mode == false) {
            ...
            struct zmk_mode_monitor_event ev = {.app_cur_mode = PPT_MODE, .state_changed = 1};
            ...
            struct zmk_mode_monitor_event ev = {.app_cur_mode = PPT_MODE, .state_changed = 0};
            k_msgq_put(&zmk_mode_monitor_msgq, &ev, K_NO_WAIT);
            k_work_submit(&mm_msg_processor.work);
        }
        ...
        if (app_mode.is_in_bt_mode == false) {
            ...
            struct zmk_mode_monitor_event ev = {.app_cur_mode = BT_MODE, .state_changed = 1};
            k_msgq_put(&zmk_mode_monitor_msgq, &ev, K_NO_WAIT);
            k_work_submit(&mm_msg_processor.work);
            ...
            struct zmk_mode_monitor_event ev = {.app_cur_mode = BT_MODE, .state_changed = 0};
            k_msgq_put(&zmk_mode_monitor_msgq, &ev, K_NO_WAIT);
            k_work_submit(&mm_msg_processor.work);
        }
        ...
        if (!gpio_pin_get_raw(win2mac.port, win2mac.pin) && app_mode.is_in_macos) {
            ...
            struct zmk_mode_monitor_event ev = {.app_cur_mode = WIN_MAC_MODE, .state_changed = 1};
            k_msgq_put(&zmk_mode_monitor_msgq, &ev, K_NO_WAIT);
            k_work_submit(&mm_msg_processor.work);
            ...
            struct zmk_mode_monitor_event ev = {.app_cur_mode = WIN_MAC_MODE, .state_changed = 0};
            k_msgq_put(&zmk_mode_monitor_msgq, &ev, K_NO_WAIT);
            k_work_submit(&mm_msg_processor.work);
        }
        ...
        if (usb_mode_monitor_trigger_level == GPIO_PIN_LEVEL_LOW) {
...
static void zmk_mode_monitor_handler(struct k_work *item) {
    ...
    while (k_msgq_get(&zmk_mode_monitor_msgq, &ev, K_NO_WAIT) == 0) {
        generate_key(6, ev.app_cur_mode, ev.state_changed);
...
int zmk_mode_monitor_init(void) {
    ...
    // if(err)
    // {
    //     LOG_ERR("Unable to disable irq of pin %u on %s,err
    //     %d",leds_pwron.pin,leds_pwron.port->name,err);
    // }
    ...
    gpio_pin_configure_dt(&ppt_irq, GPIO_INPUT | PIN_PPT_FLAGS);
    LOG_WRN("ppt port:%s,pin:%d,val:%d", ppt_irq.port->name, ppt_irq.pin,
            gpio_pin_get_raw(ppt_irq.port, ppt_irq.pin));
    if (!gpio_pin_get_raw(ppt_irq.port, ppt_irq.pin)) {
        app_mode.is_in_ppt_mode = true;
        generate_key(6, PPT_MODE, 1);
        LOG_WRN("init ppt");
        // gpio_pin_set(ppt_led.port, ppt_led.pin, 0);
    ...
    if (rc != 0) {
        LOG_ERR("configure zmk ppt  fail, err:%d ", rc);
    }
    ...
    if (rc != 0) {
        LOG_ERR("configure zmk ble  fail, err:%d ", rc);
    }
    gpio_pin_configure_dt(&bt_irq, GPIO_INPUT | PIN_BLE_FLAGS);
    LOG_WRN("bt port:%s,pin:%d,val:%d", bt_irq.port->name, bt_irq.pin,
            gpio_pin_get_raw(bt_irq.port, bt_irq.pin));
    if (!gpio_pin_get_raw(bt_irq.port, bt_irq.pin)) {
        app_mode.is_in_bt_mode = true;
        generate_key(6, BT_MODE, 1);
        LOG_WRN("init ble");
        // gpio_pin_set(bt_led.port, bt_led.pin, 0);
    ...
    gpio_pin_configure_dt(&win2mac, GPIO_INPUT | PIN_BLE_FLAGS);
    LOG_WRN("winmac port:%s,pin:%d,val:%d", win2mac.port->name, win2mac.pin,
            gpio_pin_get_raw(win2mac.port, win2mac.pin));
    if (!gpio_pin_get_raw(win2mac.port, win2mac.pin)) {
        LOG_DBG("gpio_pin_get_raw win2mac low, set to windows");
        app_mode.is_in_windows = true;
        generate_key(6, WIN_MAC_MODE, 1);
    ...
    LOG_WRN("detect_usb port:%s,pin:%d,val:%d", detect_usb.port->name, detect_usb.pin,
            gpio_pin_get_raw(detect_usb.port, detect_usb.pin));
    rc = gpio_pin_interrupt_configure_dt(&detect_usb, GPIO_INT_LEVEL_LOW);
    if (rc != 0) {
    ...
    if (is_usb_in_debonce_check) {
        ...
        if (usb_pin_polarity_status == GPIO_PIN_LEVEL_LOW) {
            ...
            if (usb_in_high_vol_num >= USB_IN_DETECT_NUM) {
                ...
                struct zmk_mode_monitor_event ev = {.app_cur_mode = USB_MODE, .state_changed = 1};
                k_msgq_put(&zmk_mode_monitor_msgq, &ev, K_NO_WAIT);
                k_work_submit(&mm_msg_processor.work);
        ...
        if (usb_pin_polarity_status == GPIO_PIN_LEVEL_HIGH) {
            LOG_DBG("[usb_mode_monitor_debounce_timeout_cb]: usb_out_low_vol is %d",
                    usb_out_low_vol_num);
            usb_out_low_vol_num++;
            if (usb_out_low_vol_num >= USB_OUT_DETECT_NUM) /* 300ms */
            {
                ...
                gpio_pin_interrupt_configure_dt(&detect_usb, GPIO_INT_LEVEL_LOW);
                usb_mode_monitor_trigger_level = GPIO_PIN_LEVEL_LOW;
                struct zmk_mode_monitor_event ev = {.app_cur_mode = USB_MODE, .state_changed = 0};
                k_msgq_put(&zmk_mode_monitor_msgq, &ev, K_NO_WAIT);
                k_work_submit(&mm_msg_processor.work);
                // app_global_data.is_app_enabled_dlps = true;
                return;
