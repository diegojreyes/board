...
void set_report_rate(uint8_t div )
{
    if(div==0)
        ...
        tap_code_delay_us = 125* (1<<div);
    // if(zmk_endpoints_selected().transport == ZMK_TRANSPORT_PPT)
    // {
    //     if(tap_code_delay_us<1000)
    //         tap_code_delay_us =0;
    // }
    LOG_ERR("tap_code_delay_us:%d",tap_code_delay_us);
    lowpower_settings();
}
uint8_t get_report_rate(void)
{
    return tap_code_delay_us /125;
}

void launcher_keycode_to_binding(uint8_t keycode,struct zmk_behavior_binding * binding);
int zmk_macro_queue_add(uint8_t code,uint8_t type ,uint32_t delay);
    ...
    while (1) {
        ...
        if (ascii_code == SS_QMK_PREFIX) {
            ...
            if (ascii_code == SS_TAP_CODE) {
                ...
                LOG_DBG("delay:%d",ms);
                zmk_macro_queue_add(0,SS_DELAY_CODE,ms*1000);
            }
        } else {
...
void send_string_end(void)
{
    ...
    launcher_keycode_to_binding(item.code,&keycode_binding);
    ...
    switch(item.type)
    {
        ...
        case SS_TAP_CODE:
        {
            uint32_t delay = (item.code == KC_CAPS_LOCK) ? (TAP_HOLD_CAPS_DELAY*1000) : TAP_CODE_DELAY;
            if(keycode_binding.param2 ==0xffff)
                delay = 100*1000;
...
int zmk_macro_queue_add(uint8_t code,uint8_t type ,uint32_t delay) {
    ...
    uint32_t default_delay =tap_code_delay_us;
    // if(zmk_endpoints_selected().transport == ZMK_TRANSPORT_PPT)
    // {
    //     default_delay =1;
    // }
    ...
    if(k_msgq_num_used_get(&zmk_macro_queue_msgq)>=2)
    {
        ...
        struct zmk_behavior_binding keycode_binding;
        launcher_keycode_to_binding(item1.code,&keycode_binding);
        switch(item1.type)
        {
            case SS_TAP_CODE:
            {
                uint32_t delay = (item1.code == KC_CAPS_LOCK) ? (TAP_HOLD_CAPS_DELAY*1000) : default_delay;
                
                zmk_behavior_queue_add(0,keycode_binding,true,delay);
                zmk_behavior_queue_add(0,keycode_binding,false,item2.delay ? item2.delay: default_delay);

            }
                break;

            case SS_DOWN_CODE:                
                zmk_behavior_queue_add(0,keycode_binding,true,item2.delay ? item2.delay: default_delay);
