...
uint8_t get_report_rate(void) { return tap_code_delay_us / 125; }
...
void send_string_end(void) {
    ...
    case SS_TAP_CODE: {
        uint32_t delay =
            (item.code == KC_CAPS_LOCK) ? (TAP_HOLD_CAPS_DELAY * 1000) : TAP_CODE_DELAY;
...
int zmk_macro_queue_add(uint8_t code, uint8_t type, uint32_t delay) {
    ...
    if (k_msgq_num_used_get(&zmk_macro_queue_msgq) >= 2) {
        ...
        case SS_TAP_CODE: {
            uint32_t delay =
                (item1.code == KC_CAPS_LOCK) ? (TAP_HOLD_CAPS_DELAY * 1000) : default_delay;

            zmk_behavior_queue_add(0, keycode_binding, true, delay);
            zmk_behavior_queue_add(0, keycode_binding, false,
                                   item2.delay ? item2.delay : default_delay);
