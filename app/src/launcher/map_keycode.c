...
uint8_t mod_to_zmk(uint8_t mod) { return (mod & 0x10) ? (mod << 4) : mod; }
...
void launcher_keycode_to_binding(uint8_t keycode, struct zmk_behavior_binding *binding) {
    ...
    case MOUSE_KEYCODE_RANGE:
        ...
        binding->param1 = keycode;
        if ((keycode >= KC_MS_UP && keycode <= KC_MS_RIGHT) ||
            (keycode >= KC_MS_WH_UP && keycode <= KC_MS_WH_RIGHT))
...
void set_zmk_keymap(uint8_t layer, uint8_t row, uint8_t column, uint16_t keycode) {
    ...
    case QK_LAYER_MOD ... QK_LAYER_MOD_MAX:
        ...
        binding.param2 = mod_to_zmk_keycode(
            QK_LAYER_MOD_GET_MODS(keycode)); // mod_to_zmk(QK_LAYER_MOD_GET_MODS(keycode))<<24;
        break;
...
void generate_launcher_keymaps(void) {
    for (int layer = 0; layer < DYNAMIC_KEYMAP_LAYER_COUNT; layer++) {
                ...
                if (position >= 0) {
                    ...
                    if (memcmp(behavior_dev, "key_press", 9) == 0) //&kp
                    {
                        ...
                        case HID_USAGE_KEY:
                            ...
                            if (mod)
                                gen_launcher_keymaps[layer][row][column] |= mod_to_launcher(mod)
                                                                            << 8;
                            break;
                        ...
                        if (mod >= HID_USAGE_KEY_KEYBOARD_LEFTCONTROL &&
                            mod <= HID_USAGE_KEY_KEYBOARD_RIGHT_GUI)
                            gen_launcher_keymaps[layer][row][column] =
                                QK_LAYER_MOD | (layer << 5) | mod_keycode_to_launcher(mod);
                        else
                            gen_launcher_keymaps[layer][row][column] = KC_NO;
        ...
        for (int i = 0; i < 2; i++) {
            uint16_t keycode = 0;
            if (memcmp(binding->behavior_dev, "rgb_ug", 6) == 0 && binding->param1 >= 1 &&
                binding->param1 <= 10) {
