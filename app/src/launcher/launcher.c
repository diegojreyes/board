...
static int zmk_usb_hid_launcher_init(void) {
    ...
    usb_hid_register_device(hid_launcher_dev, zmk_hid_launcher_report_desc, sizeof(zmk_hid_launcher_report_desc), &ops);
...
static int launcher_handle_set(const char *name, size_t len, settings_read_cb read_cb,
    ...
    if (settings_name_steq(name, "magic", NULL)) {
        ...
        LOG_DBG("magic:%02x,%02x,%02x",launcher_device.magic[0],launcher_device.magic[1],launcher_device.magic[2]);
    }
    else if (settings_name_steq(name, "layout", NULL)) {
        if (len != sizeof(launcher_device.layout_options)) {
            LOG_ERR("Invalid layout size (got %d expected %d)", len, sizeof(launcher_device.layout_options));
            return -EINVAL;
        }

        int err = read_cb(cb_arg, &launcher_device.layout_options[0], sizeof(launcher_device.layout_options));
        if (err <= 0) {
        ...
        if(layer >DYNAMIC_KEYMAP_LAYER_COUNT) return -EINVAL;
        if (len != KEYMAP_LEN) {
        ...
        if (len != sizeof(launcher_device.encoder)) {
            LOG_ERR("Invalid encoder size (got %d expected %d)", len, sizeof(launcher_device.encoder));
            return -EINVAL;
        }
        ...
        if(zmk_endpoints_selected().transport== ZMK_TRANSPORT_USB){
            if(debounce.div <=6) set_report_rate(debounce.div);
            LOG_ERR("stored usb div:%d",debounce.div);
        }
        else if(zmk_endpoints_selected().transport== ZMK_TRANSPORT_PPT){
            if(debounce.ppt_div <=6) set_report_rate(debounce.ppt_div);
...
{
    int rc;
    rc = settings_save_one("launcher/magic", (const void *)&launcher_device.magic[0], sizeof(launcher_device.magic));
    if (rc) {
...
{
    int rc;
    rc = settings_save_one("launcher/layout", (const void *)&launcher_device.layout_options[0], sizeof(launcher_device.layout_options));
    if (rc) {
        LOG_DBG("write failed:%d", rc);
    } else {
        LOG_DBG("OK.\n");
...
{
    int rc;
    rc = settings_save_one("launcher/custom", (const void *)&launcher_device.custom[0], sizeof(launcher_device.custom));
    if (rc) {
        LOG_DBG("write failed:%d", rc);
    } else {
        LOG_DBG("OK.\n");
...
{
    int rc;
    rc = settings_save_one("launcher/encoder", (const void *)&launcher_device.encoder[0], sizeof(launcher_device.encoder));
    if (rc) {
        LOG_DBG("write failed:%d", rc);
    } else {
        LOG_DBG("OK.\n");
...
void launcher_update_macros(void)
{
    ...
    if(len >0)
    {
        rc = settings_save_one("launcher/macros/datas", (const void *)&launcher_device.macros[0], len);
        if (rc) {
            LOG_DBG("write failed:%d", rc);
        } else {
            LOG_DBG("OK.\n");
...
bool launcher_eeprom_is_valid(void) {
    ...
    return (eeprom_read_byte(LAUNCHER_EEPROM_MAGIC_ADDR + 0) == magic0 && eeprom_read_byte(LAUNCHER_EEPROM_MAGIC_ADDR + 1) == magic1 && eeprom_read_byte(LAUNCHER_EEPROM_MAGIC_ADDR + 2) == magic2);
}
...
int launcher_init(void) {
...
#if defined(CONFIG_SHIELD_KEYCHRON_RS87_ANSI) || defined(CONFIG_SHIELD_KEYCHRON_K3SE2_ANSI)|| defined(CONFIG_SHIELD_KEYCHRON_K5SE2_ANSI)
    debounce.div =3;
    #if CONFIG_ADD_PPT_REPORT_RATE
    debounce.ppt_div =3;
    #endif 
    ...
    LOG_DBG("user debouce init,scan:%d,press:%d,release:%d",debounce.scan_period_ms,debounce.debounce_press_ms,debounce.debounce_release_ms);
#endif     
...
void launcher_set_layout_options(uint32_t value) {
    ...
    uint16_t target = (LAUNCHER_EEPROM_LAYOUT_OPTIONS_ADDR + LAUNCHER_EEPROM_LAYOUT_OPTIONS_SIZE - 1);
    for (uint8_t i = 0; i < LAUNCHER_EEPROM_LAYOUT_OPTIONS_SIZE; i++) {
...
__attribute__((weak)) bool launcher_command_kb(uint8_t *data, uint8_t length) {
    return false;
}
...
void launcher_zmk_rgb_matrix_get_value(uint8_t *data) {
    ...
    switch (*value_id) {
        case id_zmk_rgb_matrix_brightness: {
            value_data[0] = ((uint16_t)zmk_rgb_matrix_get_val() * UINT8_MAX) / RGB_MATRIX_MAXIMUM_BRIGHTNESS;
            break;
        }
...
void launcher_zmk_rgb_matrix_set_value(uint8_t *data) {
    ...
    switch (*value_id) {
        case id_zmk_rgb_matrix_brightness: {
            zmk_rgb_matrix_sethsv_no_save(zmk_rgb_matrix_get_hue(), zmk_rgb_matrix_get_sat(), scale8(value_data[0], RGB_MATRIX_MAXIMUM_BRIGHTNESS));
            break;
        }
        case id_zmk_rgb_matrix_effect: {
            if (value_data[0] == 0) {
...
enum {
    ...
    FACTORY_TEST_CMD_IO_TEST_START, //0x0b no param,return :result ,port0 ... port10 skip pins(bit=1 skip)
...
void io_test_set(uint8_t port, uint8_t value)
{
    ...
    if(port ==6)
    {
        ...
        {
            Pinmux_Deinit(i);
            Pad_Config(i, PAD_SW_MODE, PAD_NOT_PWRON, PAD_PULL_NONE, PAD_OUT_DISABLE,
                PAD_OUT_HIGH);
        }
    }
    else if(port ==9)
    ...
    for (uint8_t i = 0; i < 8; i++) {
        ...
        // 设置 pinmux 为 GPIO 模式
        Pad_Config(pin, PAD_PINMUX_MODE, PAD_IS_PWRON, 
                    value?PAD_PULL_UP:PAD_PULL_DOWN, PAD_OUT_ENABLE, value?PAD_OUT_HIGH:PAD_OUT_LOW);
        Pinmux_Config(pin, DWGPIO);

        GPIO_WriteBit(GPIO_GetPort(pin),GPIO_GetPin(pin),(value & (1 << i))?Bit_SET:Bit_RESET);
    }
}
uint8_t io_test_read(uint8_t port)
{
    ...
    for (uint8_t i = 0; i < 8; i++) {
        ...
        Pinmux_Config(pin, DWGPIO);
        Pad_Config(pin, PAD_PINMUX_MODE, PAD_IS_PWRON,PAD_PULL_NONE, PAD_OUT_DISABLE,PAD_OUT_HIGH);
...
void kc_user_cmd(uint8_t *command_data)
{
    ...
    switch(cmd)
    {
        ...
        case kc_user_cmd_debounce:
            ...
            command_data [3] = debounce.debounce_press_ms;
            command_data [4] = debounce.debounce_release_ms;
            LOG_ERR("kc read debounce[%d,%d]",debounce.debounce_press_ms,debounce.debounce_release_ms);
            break;
        case kc_user_cmd_set_debounce:
            ...
            else
            {
                ...
                user_set_debounce(debounce.debounce_press_ms,debounce.debounce_release_ms);
                LOG_ERR("kc set debounce[%d,%d]",debounce.debounce_press_ms,debounce.debounce_release_ms);
                launcher_save_debounce();
                command_data[1] =0;
                command_data[2] =0;
            }
...
uint8_t get_report_rate_div(void)
{
    return debounce.div;
}
