...
static uint32_t last_timer_c = 0;
static uint32_t last_timer_w = 0;
...
int launcher_mouse_listener(const zmk_event_t *eh) {
    const struct zmk_mouse_button_state_changed *ev = as_zmk_mouse_button_state_changed(eh);
    if (ev) {
        if (ev->state) {
            mouse_listener_keycode_pressed(ev);
        } else {
            mouse_listener_keycode_released(ev);
        }
    }
    return 0;
}
ZMK_LISTENER(launcher_mouse_listener, launcher_mouse_listener);
ZMK_SUBSCRIPTION(launcher_mouse_listener, zmk_mouse_button_state_changed);
