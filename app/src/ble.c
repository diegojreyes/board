...
// BT_LE_ADV_PARAM(BT_LE_ADV_OPT_CONNECTABLE | BT_LE_ADV_OPT_ONE_TIME | BT_LE_ADV_OPT_USE_IDENTITY,
...
// swift pair
    ...
    BT_DATA_BYTES(BT_DATA_MANUFACTURER_DATA, 0x06, 0x00, 0x03, 0x00, 0x80, 'L', 'E', ' ', 'K', 'e',
                  'y', 'b', 'o', 'a', 'r', 'd')
#else
...
#define CHECKED_OPEN_ADV()                                                                         \
    ...
    if (zmk_keymap_highest_layer_active() >= 2 && (adv_state == ZMK_ADV_PAIR)) {                   \
        err = bt_le_adv_start(&zmk_adv_conn_name, zmk_ble_ad_win, ARRAY_SIZE(zmk_ble_ad_win),      \
...
int update_advertising(void) {
    ...
    if (adv_state == ZMK_ADV_RECONN) {
        ...
        raise_zmk_led_state_changed(
            (struct zmk_led_state_changed){.led_state = LED_PEER_STATE_RECONN,
...
int zmk_ble_prof_prev(void) {
    LOG_DBG("select previous profile %d",
            (active_profile + ZMK_BLE_PROFILE_COUNT - 1) % ZMK_BLE_PROFILE_COUNT);
...
static void connected(struct bt_conn *conn, uint8_t err) {
    ...
    raise_zmk_led_state_changed(
        (struct zmk_led_state_changed){.led_state = LED_PEER_STATE_CONNECTED,
                                       .transport = ZMK_TRANSPORT_BLE,
                                       .index = active_profile});
...
static void disconnected(struct bt_conn *conn, uint8_t reason) {
    ...
    raise_zmk_led_state_changed(
        (struct zmk_led_state_changed){.led_state = LED_PEER_STATE_DISCONNECTED,
                                       .transport = ZMK_TRANSPORT_BLE,
                                       .index = active_profile});
...
static void security_changed(struct bt_conn *conn, bt_security_t level, enum bt_security_err err) {
    ...
    if (!err) {
        ...
        if (err == BT_SECURITY_ERR_PIN_OR_KEY_MISSING || err == BT_SECURITY_ERR_AUTH_REQUIREMENT) {
...
static void load_identities(void) {
    ...
    for (i = 0; i < ZMK_BLE_PROFILE_COUNT; i++) {
        if (bt_addr_le_cmp(&profiles[i].peer, BT_ADDR_LE_ANY)) {
            ...
            } else if (bt_keys_find(BT_KEYS_LTK | BT_KEYS_LTK_P256, id + 1, &profiles[i].peer) !=
                       NULL) {
...
static uint8_t get_reconn_bt_id(void) {
    ...
    if (!bt_addr_le_cmp(&profiles[active_profile].peer, BT_ADDR_LE_ANY)) {
        ...
        if (bt_keys_find(BT_KEYS_LTK | BT_KEYS_LTK_P256, profiles[active_profile].bt_id,
                         &profiles[active_profile].peer) != NULL) {
            return profiles[active_profile].bt_id;
        } else {
            if (bt_keys_find(BT_KEYS_LTK | BT_KEYS_LTK_P256, id, &profiles[active_profile].peer) !=
                NULL) {
                return id;
            } else if (bt_keys_find(BT_KEYS_LTK | BT_KEYS_LTK_P256, id + 1,
                                    &profiles[active_profile].peer) != NULL)
                return id + 1;
            else {
...
static uint8_t get_pair_bt_id(void) {
    ...
    if (!bt_addr_le_cmp(&profiles[active_profile].peer, BT_ADDR_LE_ANY))
        ...
        for (int i = 0; i < 2; i++) {
            if (bt_keys_find(BT_KEYS_LTK | BT_KEYS_LTK_P256, id + i,
                             &profiles[active_profile].peer) != NULL) {
...
void adv_timeout_work_callback(struct k_work *work) {
    ...
    if (adv_state == ZMK_ADV_RECONN) {
        k_work_reschedule(&sleep_work, last_connected ? K_MSEC(10000 - ADV_RECONN_TIME_OUT)
...
bool force_to_sleep(void) { return force_sleep; }
...
//     int rc =load_immediate_value("delay_clear_bond", &delay_clear_bond,
//     sizeof(delay_clear_bond)); if (rc == -ENOENT) {
//         delay_clear_bond = 0;
...
void update_conn_param_worker(struct k_work *work) {
    ...
    if (conn != NULL) {
        ...
        struct bt_conn_info info;
        bt_conn_get_info(conn, &info);
        LOG_ERR("conn interval:%d,latency:%d,timeout:%d", info.le.interval, info.le.latency,
                info.le.timeout);
...
uint8_t zmk_ble_is_ready(void) { return profiles[active_profile].connected; }