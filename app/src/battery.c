...
static int zmk_battery_update(const struct device *battery) {
    ...
    {
        if((state_of_charge.val1 < 90) && (voltage > BAT_VOLTAGE_LOW) && zmk_rgb_matrix_is_enabled())
        {
...
static void zmk_battery_start_reporting() {
    if (device_is_ready(battery)) {
        if(!bat_shutdown)        
            k_timer_start(&battery_timer, K_SECONDS(2), K_SECONDS(CONFIG_ZMK_BATTERY_REPORT_INTERVAL));
        else
