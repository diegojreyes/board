 ...
 */
 ...
 static const struct led_effect led_peer_state_effect[] = 
 {
     ...
     [LED_PEER_STATE_RECONN] = LED_EFFECT_LED_FLASH(3, 200,LED_COLOR(255, 255, 0)),// LED_EFFECT_LED_BLINK(200, LED_COLOR(150, 0, 0)),
     ...
     [LED_PEER_STATE_BAT_LOW] = LED_EFFECT_LED_FLASH3(5,LED_COLOR(255, 255,255)),//LED_EFFECT_LED_BLINK(1000, LED_COLOR(255, 0,0)),
 ...
 static void work_handler(struct k_work *work)
 {
    ...
    //  LOG_INF("led %d,time:%d,%d",LED_ID(led),led->effect->steps[0].substep_time,led->effect->steps[1].substep_time);
     ...
     if (led->effect_substep == effect_step->substep_count) {
         ...
         if (led->effect_step == led->effect->step_count) {
             if (led->effect->loop_forever) {
                 ...
                 if(LED_ID(led)==led_ble_index || (LED_ID(led)>=led_ble0_index && LED_ID(led)<(led_ble0_index+3)))
                 {
                 ...
                 if(led->effect == &led_peer_state_effect[LED_PEER_STATE_POWER_ON] && (LED_ID(led)>=(led_ble0_index+2)))
#else
                 if(led->effect == &led_peer_state_effect[LED_PEER_STATE_POWER_ON] && (LED_ID(led)>=(led_ble_index)))
#endif                  
                 {
...
#if (!defined(CONFIG_SHIELD_KEYCHRON_RS87_ANSI) && !defined(CONFIG_SHIELD_KEYCHRON_Q1_ULTRA_ANSI)&& !defined(CONFIG_SHIELD_KEYCHRON_Q3_ULTRA_ANSI)&& !defined(CONFIG_SHIELD_KEYCHRON_Q6_ULTRA_ANSI)&& !defined(CONFIG_SHIELD_KEYCHRON_V6_ULTRA_ANSI)&& !defined(CONFIG_SHIELD_KEYCHRON_V3_ULTRA_ANSI)&& !defined(CONFIG_SHIELD_KEYCHRON_V5_ULTRA_ANSI)&& !defined(CONFIG_SHIELD_KEYCHRON_V1_ULTRA_ANSI)&& !defined(CONFIG_SHIELD_KEYCHRON_V10_ULTRA_ANSI))	                     
                     if(zmk_endpoints_selected().transport ==ZMK_TRANSPORT_BLE)
                     {
                         if(zmk_ble_active_profile_is_connected())
                         {
                             LOG_ERR("restore led connected");
                             gpio_led_blue_set_state(LED_PEER_STATE_CONNECTED,zmk_ble_active_profile_index());
                         }
                         else
                         {
     ...
     if (led->effect_step < led->effect->step_count) {
        ...
        //  LOG_INF("led %d,led->effect_step:%d,next delay:%d",LED_ID(led),led->effect_step,next_delay);
 ...
 void gpio_led_blue_set_state(uint8_t led_state,uint8_t index)
 {
     ...
     // if((leds[LED_BLUE+index].effect !=&led_peer_state_effect[led_state]) || k_work_delayable_remaining_get(&leds[LED_BLUE+index].work)==0)
     {
 ...
 void gpio_led_24G_set_state(uint8_t led_state)
 {
     ...
     // if((leds[LED_24G].effect !=&led_peer_state_effect[led_state]) || k_work_delayable_remaining_get(&leds[LED_24G].work)==0)
     {
 ...
 void gpio_led_power_on(void)
 {
     ...
     if(bat_is_shutdown()) return;
     if(led_indicator_state.power_on) return;
     // is_app_enabled_dlps =false;
     led_indicator_state.running =1;
     led_indicator_state.exclude =1;
...
uint8_t keyboad_gpio_led_set_onoff(uint8_t led_state)
{
	...
	if(led_indicator_state.power_on) return 1;
...
{
    return led_indicator_state.power_on;
}
