...
void rgb_set_state(uint8_t type, uint8_t led_state, uint8_t index) {
    ...
    LOG_ERR("state:%d,flash count:%d,on:%d,off:%d\n", led_state, rgb_led_indicators.led_flash_count,
            rgb_led_indicators.led_on_time, rgb_led_indicators.led_off_time);
    zmk_rgb_led_indicatots_on();
}

void blue_led_set_state(uint8_t led_state, uint8_t index) {
    static uint8_t last_index = 0;
    LOG_ERR("blue_led_set_state:%d,cur run:%d,exclude:%d", led_state, rgb_led_indicators.running,
            rgb_led_indicators.exclude);
...
void led_24G_set_state(uint8_t led_state) {
    LOG_ERR("led_24G_set_state:%d,cur run:%d,exclude:%d", led_state, rgb_led_indicators.running,
            rgb_led_indicators.exclude);
    if ((rgb_led_indicators.running && rgb_led_indicators.exclude)
#if 0 // CONFIG_SHIELD_KEYCHRON_RS87_ANSI
        || gpio_led_is_power_on()
...
//  zmk_rgb_led_indicatots_on();
// }
uint8_t led_is_display_bat(void) { return rgb_led_indicators.bat_info; }
...
hid_led_t keyboard_get_led_state(void) { return keyboard_led_state; }
...
uint32_t aon_read_reg(void) { return AON_REG_READ(AON_NS_REG15X_APP); }
void aon_write_reg(uint8_t value) { AON_REG_WRITE(AON_NS_REG15X_APP, value); }
