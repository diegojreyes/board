...
enum PMCheckResult app_enter_dlps_check(void) {
    ...
    && bat_enable_dlps 
    && settings_enabled_dlps
    && macro_enabled_dlps
    && kscan_enabled_dlps
#ifdef CONFIG_ZMK_SSD1306    
    && i2c_is_allow_dlps()
#endif     
    ) ? PM_CHECK_PASS : PM_CHECK_FAIL;
}
...
int set_cpu_noraml_freq(void)
{
    return pm_cpu_freq_set(40, &actual_mhz);
}
...
void lowpower_settings(void)
{
    ...
    LOG_ERR("trans:%d,target cpu freq:%d,now:%d,dlps:%d,report rate:%d",zmk_endpoints_selected().transport,target_cpu_freq,actual_mhz,is_app_enabled_dlps,get_report_rate());
}
