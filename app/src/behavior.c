...
static int check_behavior_names(void) {
    ...
    // LOG_ERR("__zmk_behavior_ref_rom_start:%p,start:%p,end:%p,size:%d",(uint8_t
    // *)__zmk_behavior_ref_rom_start,_zmk_behavior_ref_list_start,_zmk_behavior_ref_list_end,(uint32_t)_zmk_behavior_ref_list_end
    // - (uint32_t)_zmk_behavior_ref_list_start);

    memcpy(_zmk_behavior_ref_list_start, __zmk_behavior_ref_rom_start,
           (uint32_t)_zmk_behavior_ref_list_end - (uint32_t)_zmk_behavior_ref_list_start);
    ptrdiff_t count;
