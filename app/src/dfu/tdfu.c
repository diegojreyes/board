...
static unsigned char fw_sc_upgrade_over_myhid_write_to_flash(unsigned char *pdata ,unsigned char len)
{
	...
	if(sc_dfu_wait_reboot)
		...
		DBG_DIRECT("dfu wr(%d,%x)[crc=%x],len=%d:,%b",sc_dfu_security_mode,dfu_location,Dfu_CRC_Val,len,
			TRACE_BINARY(len, pdata));
	#else
		APP_PRINT_INFO5("dfu wr(%d,%x)[crc=%x],len=%d:,%b",sc_dfu_security_mode,dfu_location,Dfu_CRC_Val,len,
			TRACE_BINARY(len, pdata));
	#endif
#endif
...
static unsigned char cfu_get_patch_version_and_bank_num(unsigned char *pdata,unsigned char len)
{
	...
	for(unsigned char i=0;i<MAX_IMG_ID_NUMERS;i++)
		...
		APP_PRINT_INFO6("[cfu_get_patch_version_and_bank_num,%d],image id=%x, version = %d.%d.%d.%d",
			i,image_id[i],
...
void my_scdfu_data_handle(unsigned char *pdata,unsigned char rxlen)
{
	...
	#else
	APP_PRINT_INFO4("fw_sc_upgrade_wr,len=%d,op_code=%02x,sum_rx=%04x,sum_cal=%04x,\r\n",len_of_dfu,op_code,sum_rx,sum);
	#endif
#endif
				switch(op_code)
					{
